...
def main():
    ...
    warped_color0 = utils.forward_warp_vectorize(ys_color1_t, ys_mv1_t)
    diff_color0 = (ys_color0_t - warped_color0).abs().clamp(0.0, 1.0)
    ssim_color0 = float(utils.ssim(ys_color0_t, warped_color0).detach().cpu())

    # 反向 warp: backward_warp(C0, F0<-1) => C1，但原神没有 F0<-1，这里用 F1->0 近似
    warped_color1 = utils.backward_warp_gridsample(ys_color0_t, ys_mv1_t)
    diff_color1 = (ys_color1_t - warped_color1).abs().clamp(0.0, 1.0)
    ssim_color1 = float(utils.ssim(ys_color1_t, warped_color1).detach().cpu())
    ...
    axes[0, 1].set_title("MV 0")
    axes[0, 2].imshow(_to_color_rgb(ys_color1))
    axes[0, 2].set_title("Color 1")
    axes[0, 3].imshow(ys_mv1_rgb)
    axes[0, 3].set_title("MV 1")

    axes[1, 0].imshow(_to_color_rgb(warped_color0))
    axes[1, 0].set_title("Forward Warped Color 0 <-- 1")
    ...
    axes[1, 2].set_title("Backward Warped(Approx.) Color 1 <-- 0")
